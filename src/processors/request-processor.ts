...
export const processRequest = async (
  ...
  if (action === Constants.WDWNT_WELCOME || action === 'DefaultWelcomeIntent') {
    ...
    action === Constants.ATTRACTION_WAIT ||
    action === 'WaitTimeForAttraction'
  ) {
    return await processWaitTimeRequest(parameters.attraction);
  } else if (
    action === Constants.ENTERTAINMENT_NEXTSHOW ||
    action === 'NextShowtimeForEntertainment'
  ) {
    ...
    action === Constants.BLOG_LATEST_POSTS ||
    action === 'LatestHeadlines'
  ) {
    return await processLatestHeadlinesRequest();
  } else if (
    action === Constants.PODCAST_LISTEN ||
    action === 'LatestPodcast'
  ) {
    ...
    action === Constants.CHARACTER_APPEARANCES ||
    action === 'MeetAndGreets'
  ) {
...
async function processWaitTimeRequest(attractionKey: string) {
  ...
  const result = await pool.query(
    `select top 1 * from AttractionInfos inner join Images on FacilityId = AttractionInfos.Id where LanguageProcessingKey = '${attractionKey}'`
  );
  pool.close();

  if (result.recordset.length === 0) {
    const noMatchingAttractionsResponse = `Sorry! We couldn't find an attraction that matches your request!`;
    const responseObject = buildResponse(
      noMatchingAttractionsResponse,
      noMatchingAttractionsResponse
    );
  ...
  addCardResponse(
    ...
    attraction.Name,
    attraction.CurrentStatus,
    attraction.Description,
    imageUrl
  );

  const moreInfoUrl = `https://now.wdwnt.com/attraction/details/${attraction.Id[0]}`;
  addButtonToCardResponse(
    response.payload.google.richResponse.items[1],
    'More info',
    moreInfoUrl
  );
...
async function processParkHoursRequest(parkKey: string, date: Date) {
  ...
  const result = await pool.query(
    `select * from ParkInfos where Abbreviation = '${parkKey}'`
  );
  pool.close();
  ...
  if (park.TodaysHours.toLowerCase().indexOf('closed') === -1) {
    const parkHours = park.TodaysHours.replace('<br />', '')
...
async function processNTunesListenRequest() {
  ...
  const ntunesInfo = {
    ...
    displayText,
    featured_image: 'https://sir.wdwnt.com/app/WDWNTunes_v3_600.png',
...
async function processWeatherRequest(destination: string) {
  const json = (await downloadJson(
    `https://weather.wdwnt.com/api/speech/${destination}`
  )) as WeatherResponse;
...
function processCornacRequest() {
  ...
  const responseObject = buildResponse(
    cornacResponse.response,
    cornacResponse.displayText
  );
  addResponseRequest(responseObject);
...
async function processCharacterAppearancesRequest(characters: string) {
  ...
  if (json.length === 0) {
    const noMeetAndGreetsResponse =
      'There are no meet and greets listed today for that character.';
    const noMeetAndGreetsResponseObject = buildResponse(
      noMeetAndGreetsResponse,
      noMeetAndGreetsResponse
    );
  ...
  const response = json
    .map(
      (ca: MeetAndGreetResponse) =>
        `${ca.Location} at ${ca.ParkName} from ${ca.NextAppearanceDisplay}`
    )
    .join(', ');
...
function addButtonToCardResponse(
  cardResponse: any,
  buttonTitle: string,
  buttonUrl: string
) {
